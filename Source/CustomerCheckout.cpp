...
//using namespace std;
//using std::string;
//using distribution = std::uniform_int_distribution<>;//产生随机分布
//
//// Output histogram of service times
//void histogram(const std::vector<int>& v, int min)
//{
//    string bar (60, '*');   // Row of asterisks for bar生成60个星号
...
//
//    // Setup minimum & maximum checkout periods - times in minutes//设定最短和最长的结账时间
//    int service_t_min {2}, service_t_max {15};
//    distribution service_t_d {service_t_min, service_t_max};//在这个范围内产生随机数
//
//    // Setup minimum & maximum number of customers at store opening
//    int min_customers {15}, max_customers {20};//设定最少顾客数和最多顾客数
...
//    }
//
//    std::vector<Checkout> checkouts {n_checkouts}; //根据输入的数值，初始化等待结账的队伍个数
